...
TransitionCache = dict[
    int,
    list[tuple] | dict[tuple, list[tuple]],
]


def _is_grouped_coalition_cache(transition_cache: dict | None) -> bool:
...
def build_transition_cache(
    cgs: CGSProtocol, coalition: str | None = None
) -> TransitionCache:
    ...
    coalition_context: tuple[set[int], int] | None = None
...
def _all_opponent_moves_lead_to_target(
    ...
    use_bit_vector: bool,
    target_bits: BitVectorStateSet | None,
