"""
Formula tree solver for RABATL model checking.

This module contains the solve_tree function that recursively evaluates
RABATL formula trees using bottom-up evaluation.
"""

from model_checker.algorithms.explicit.RABATL.operators import (
...
def _rabatl_unary_key(parser_instance, val):
...
def _rabatl_binary_key(parser_instance, val):
...
def solve_tree(cgs, node, cache=None):
    """
    Recursively solve the RABATL formula tree.
    """
    if cache is None:
        cache = {}
    ...
    if node.right is None:
        key = _rabatl_unary_key(_get_parser(), val)
        if key and key in _UNARY:
            _UNARY[key](cgs, node)
    elif node.left and node.right:
        key = _rabatl_binary_key(_get_parser(), val)
