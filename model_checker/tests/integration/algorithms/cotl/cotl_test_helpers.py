...
def initial_state_name_from_path(path):
    ...
    return str(parser.initial_state) if parser.initial_state else None
...
def states_where_prop_holds(model_path, prop):
    ...
    matrix = parser.matrix_prop
    idx = proposition_index(parser.atomic_propositions, prop)
    if idx is None:
        return set()
    return {
